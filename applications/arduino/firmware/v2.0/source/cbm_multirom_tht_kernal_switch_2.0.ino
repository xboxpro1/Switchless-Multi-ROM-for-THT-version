...
void setup() {                                              // setup
  ...
  #else
    FastGPIO::Pin<pin8>::setInputPulledUp();                // according to the avr data sheet it is recommended to ensure that unused pins have a defined level
  #endif

  FastGPIO::Pin<pin9>::setInputPulledUp();                  // according to the avr data sheet it is recommended to ensure that unused pins have a defined level
  FastGPIO::Pin<pin10>::setInputPulledUp();                 // according to the avr data sheet it is recommended to ensure that unused pins have a defined level

  //FastGPIO::Pin<pin9>::setInput();                        // connected to ROM O!E signal
  //FastGPIO::Pin<pin10>::setInput();                       // connected to ROM A13 signal
      
  savedROM = EEPROM.read(0);                                // retrieve last used ROM from AVR MCU EEPROM and switch ROM using address lines A13-A18
